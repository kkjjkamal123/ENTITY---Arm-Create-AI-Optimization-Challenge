...
**Demo video: <https://youtu.be/ZD_jpyBqkF8>** — the app running on the reference phone.
...
Llama-3.2-1B-Instruct-Q4_0, PP 512 / TG 128, unplugged, five runs per arm, 2026-07-18. Raw
per-pass CSVs in [`benchmarks/results/`](benchmarks/results/).

| | CMF Phone 1 (Dimensity 7300) | OPPO CPH2729 (Snapdragon 6 Gen 4) |
|---|---:|---:|
| Decode, naive 8 threads | 10.8 tok/s | 9.7 tok/s |
| **Decode, ENTITY Auto** | **18.1 tok/s (+68%)** | **17.5 tok/s (+81%)** |
| Attribution: thread count | +39% | +80% |
| Attribution: core pinning | +21% | +1% (but 2.52 → 1.78 W) |
...
- **v3.6.2** (2026-07-24): **the assistant had no idea it was ENTITY.** The default system prompt said only that it was "a helpful AI assistant running fully offline" - true, but thin enough that a small model would answer identity questions as whatever base persona it was trained on, or guess at capabilities (web search, image generation) it structurally does not have. Neither gap was hypothetical: every model in the catalog is text-only, and `ai_chat.cpp` has no image pipeline and makes no network call anywhere in the inference path. `Settings.kt`'s `DEF_SYSTEM_PROMPT` now states plainly what ENTITY is, what it cannot do, and to say so instead of guessing when asked - the existing no-roleplay/no-sound-effects constraint (added earlier after that exact failure mode) is unchanged. A default only: existing users keep their current prompt until they reset it, and ENTITY Bench has no chat surface so this is chat-only (`apk/ENTITY-v24-entity-identity-prompt-20260724-release.apk`) - release-signed and since run on device across the contributed benchmark fleet listed on the [leaderboard](https://kkjjkamal123.github.io/ENTITY-WEB/).
- **v3.6.1 + ENTITY Bench v2.1.1** (2026-07-24): **repetition penalty was off.** `new_sampler()` set `temp`, `top_k` and `top_p` from the user's config and left everything else - including `penalty_repeat` - at llama.cpp's own `common_params_sampling` defaults, and that struct's own default for `penalty_repeat` is `1.0`: disabled. Paired with ENTITY's deliberately conservative `temp = 0.3`, the sampler ran close to greedy decoding, the textbook setup behind two complaints that came back together - a chat that loops and a chat that reads bland are not opposite symptoms, they are the same collapse (Holtzman et al., *The Curious Case of Neural Text Degeneration*). `penalty_repeat = 1.1` fixes it without raising temperature, which would have traded against the factual grounding `temp = 0.3` was chosen for; not yet a user-facing setting. Bench v2.1.1 mirrors the identical sampler change, so its numbers keep measuring what chat actually ships, plus a Settings fix: the Contribute section's on/off toggle was styled identically to the two navigation buttons beside it (no visual cue it was a persistent switch rather than another button to tap), and a dead duplicate button with no click listener at all sat right next to the working one. Both are fixed - the toggle is now a checkbox row with an explicit description, matching the pattern the chat app already uses for its own settings, and the dead button is deleted. Addresses looping and blandness directly; does not address the reported long-chat coherence loss, which is `shift_context()`'s discard-oldest-half strategy - a separate, still-open issue (`apk/ENTITY-v23-repeat-penalty-20260724-release.apk`, `apk/ENTITY-Bench-v2.1.1-release.apk`) - release-signed and run on device; Bench v2.1.1 results from contributed devices are in the dataset (see [`benchmarks/CONTRIBUTED-DATA.md`](benchmarks/CONTRIBUTED-DATA.md)).
- **v3.6.0 + ENTITY Bench v2.1.0** (2026-07-23): **the app now tells the kernel its deadline, not just which cores to use - and a power bug that made watts meaningless is fixed.** An unprivileged Android app cannot do kernel optimization: cpufreq governors, scheduler policy, `/proc/sys` and realtime priority all need root, and a rooted app would be unshippable and its benchmarks unrepresentative. There is exactly one sanctioned route into those subsystems and v3.6.0 takes it. `sched_setaffinity` says *where* work runs but cannot say *how fast it must be*, so the kernel still picks a frequency by reacting to load after the fact - and a hard mask stops the platform migrating work as the phone heats up, which is why Android's own guidance is to avoid manual affinity. An **ADPF performance hint session** declares the deadline for each decode step instead, and the framework feeds that to the same scheduler and cpufreq machinery the vendor already tuned. That matters most for the decision the contributed data showed to be genuinely device-dependent: pinning ranged from -8.5% to +29.3% on decode and was slightly *negative* on tokens per watt in the median, so moving it to the device is the right place for it. Core APIs are `__INTRODUCED_IN(33)` - the app's minSdk - so they link with no runtime guard, and every call degrades to a no-op where a vendor did not implement the HAL. Stated limitation: a session boosts the threads registered in it, and ggml's workers expose no enumerable TID, so only the thread running `llama_decode` is registered. **No speedup is claimed** - Bench v2.1.0 adds an `adpf` arm (Auto's thread count, affinity off, deadline on) so the hint is measured against `threads_only` and `optimized` rather than assumed, which is the rule every other claim here had to satisfy. Also fixed: battery power under-reported by **1,000,000x** on an OPPO CPH2737 (Dimensity 8300, not Snapdragon). The cause was voltage, not current - `EXTRA_VOLTAGE` is documented in millivolts and that device reports whole volts, so `PowerMath`'s microamp/milliamp heuristic found both candidates below the plausible floor and fell through to the documented unit; two independent 1000x errors compounding into 2.7 microwatts of decode and 11 million tok/W. `normalizeVoltageMv()` resolves the voltage unit first. The affected contributed rows keep their throughput and lose their power (`apk/ENTITY-v22-adpf-power-fix-20260723-release.apk`, `apk/ENTITY-Bench-v2.1.0-release.apk`).

- **v3.5.0 + ENTITY Bench v2.0.0** (2026-07-23): **prompt processing was running on two threads on every flagship, and core placement stopped being an assumption.** Six contributed rows across four SoCs showed what two development phones could not: a Dimensity 7300 prefills Llama-3.2-1B-Q4_0 at 139 tok/s while an SM8550 - stronger silicon, with i8mm - manages 111. `top_cluster_core_count()` counts cores within 10% of the fastest, a proxy for "the performance cluster" that only holds on chips with no prime core; every modern flagship puts its prime 17-20% above its own big cluster (3052/3782 on Tensor G5, 2803/3360 on SM8550, 2496/2995 on SM8450), so the count collapsed to 1 and only `N_THREADS_MIN` pulled it back to 2 - and prefill inherited that number because `n_pp = n_gen`. Cluster detection now reads `/sys/devices/system/cpu/cpuN/cpu_capacity`, the kernel's own normalised per-core capacity and the signal the scheduler itself uses, falling back to `cpuinfo_max_freq` where a kernel omits it; frequency cannot separate an A55 at 2.0 GHz from an A78 at 2.5 GHz, 25% apart in clock and ~3x apart in throughput. Prefill now runs on the performance cluster - every core above the slowest tier, which resolves to 4/6/5/4 across those four devices under both the capacity signal and the frequency fallback - while the **decode thread count is deliberately unchanged** (n_gen 4/2/2/2), because decode is bandwidth-bound and 2 threads already saturates it - an SM8550 decodes 23.8 tok/s on 2 threads and 6.72 on 8. Second change: **core placement is now the user's choice** (Settings -> Inference: AUTO / PERF CORES / SCHEDULER). Pinning swings from -8.5% to +29.3% on decode across the dataset and is slightly *negative* on tok/W in the median - the Pixel 10 is the clean case, +29.3% faster for +33.5% more power - and Android's own guidance is that forcing affinity stops the platform reacting to load and thermal throttling. The ablation already runs threads-only and optimized at the same thread count differing only in affinity, so Settings reports which won *on this phone* and offers one-tap apply, with the energy half suppressed for charging runs whose watts are the charger's. Chat also gains **LaTeX rendering** - `$..$`, `$$..$$`, `\(..\)`, `\[..\]`, Unicode where it suffices and custom Canvas spans for stacked fractions and radicals, zero new dependencies. Bench v2.0.0 carries the identical inference change (so its `optimized` arm still measures what chat ships) and adds `cpu_capacities` to the upload payload. (`apk/ENTITY-v21-prefill-threads-placement-latex-20260723-release.apk`, `apk/ENTITY-Bench-v2.0.0-release.apk`) - release-signed and run on device; results from Bench v2.0.0 onward are in the contributed dataset (see [`benchmarks/CONTRIBUTED-DATA.md`](benchmarks/CONTRIBUTED-DATA.md)).

- **v3.4.1 + ENTITY Bench v1.5.0** (2026-07-22): **edge-to-edge layout fixed in both apps, and the bench app brought onto the chat app's colour system.** Every layout carried `android:fitsSystemWindows="true"` and it was inert - from targetSdk 35 Android draws apps edge-to-edge and stops honouring that attribute on ordinary containers, so content ran under the system bars and text sat against the display cutout. Real window-inset padding now covers system bars and cutout on every screen, adds to each layout's own gutter instead of replacing it, sits on containers so lists still scroll *under* the bars, and lets the chat input rise with the keyboard. Bench v1.5.0 takes the v3.4.0 palette verbatim (the palette files are copied byte-identical so the two apps stay diffable): #121212/#E4E4E4 dark, off-white light, 1dp outline borders, and a MONOCHROME / COLOUR switch in Settings. Bench also now detects **SME2** as its own `/proc/cpuinfo` flag and shows an `sme2 kleidiai` chip - a reporting fix, not a new capability: the shipped `android_armv9.2_2` variant already carried SVE2+SME and KleidiAI's SME2 microkernels are compiled in whenever `+sme` is set, so those kernels always ran on SME2 silicon. Seven CPU variants ship in total, armv8.0 through armv9.2+SVE2+SME, selected at runtime - so any arm64 phone from pre-dotprod to SME2 gets the strongest kernels it can run. No measurement or inference-path changes in either app (`apk/ENTITY-v20-edge-insets-20260722-release.apk`, `apk/ENTITY-Bench-v1.5.0-release.apk`).
- **v3.4.0** (2026-07-22): **the colour system, rebuilt on evidence rather than taste.** Every version through v3.3.0 painted pure `#FFFFFF` on pure `#000000` - a 21:1 contrast ratio. On a black field the iris opens wider and white glyphs bleed into their own edges (halation): it reads as glare within minutes and is worst for the roughly half of people with some astigmatism, which is why Material's dark-theme guidance is never to use black as the base surface. Dark is now `#121212` / `#1E1E1E` / `#E4E4E4` - Material's dark-surface baseline and its 87% high-emphasis text level - and light is `#F1F0EC` paper with cards *lighter* than the page, following Carbon's alternating layering model rather than piling white on white. Body contrast falls 21:1 -> 15:1, still above WCAG AAA. Two changes mattered as much as the hex values: card borders were 2dp at full text strength (now 1dp of a dedicated outline tone) and large filled buttons are now deliberately dimmer than text, because area multiplies perceived glare. Colour is now addressed by **role, as theme attributes**, so raw colour resources appear only inside the two palette themes - which is what makes the new **Settings -> Theme -> MONOCHROME / COLOUR** switch possible. Monochrome stays the default; Colour changes only hue, with semantic tones deliberately separate from the accent so identity is never confusable with warning, and every coloured state still carries text or shape (WCAG 1.4.1). Also fixes a model being unreloadable after reopening the app: the active-model preference persists across restarts but the engine does not, so the last model showed as LOADED on a disabled button with nothing loaded. No inference-path, thread-derivation or pinning changes; every published CMF and OPPO result carries over (`apk/ENTITY-v19-models-screen-colour-20260722-release.apk`).
- **v3.3.0** (2026-07-22): **Models get a screen instead of a dialog.** v3.2.0 put the catalog into an alert dialog whose every entry was four or five lines of wrapped prose stacked with no separation - the information was right and unreadable. A real Models screen now splits *On this phone* from *Available to download*, with one card layout for both so each fact lands in the same place: name, a fixed facts line, KleidiAI and fit pills, the reason, then actions (LOAD / DELETE, or DOWNLOAD / RESUME with a progress bar). A downloaded catalog entry is no longer listed twice, and a card carries at most one solid emphasis - `ACTIVE` or `RECOMMENDED` - because solid inversion is this design's strongest mark. No inference-path changes.
- **v3.2.0** (2026-07-22): **model catalog - a fresh install now has something to talk to.** ENTITY always required the user to supply their own `.gguf`: find a model host in a browser, judge a quantization and a parameter count against the phone by eye, download it, then import it through the file picker - a reasonable ask of a developer and the single step where a first run most often ended for everyone else. The model picker now offers a curated catalog of seven Qwen2.5 and Llama 3.2 GGUFs, each row tagged for the phone in hand (RECOMMENDED / GOOD FIT / FITS / TIGHT / TOO BIG) with a one-line reason naming the quantization and the ISA it will actually reach on that CPU - read from the backend variant ggml dlopened for the device, so the row describes kernels that will really run rather than what the silicon nominally supports. Fit is judged against total RAM, not free RAM, so the same phone cannot give a different verdict minute to minute. Downloads are resumable (bytes land in a `.part` and a retry continues with an HTTP `Range` request) and the file only takes its real `.gguf` name once its length matches the catalog's expected size, so a truncated transfer can never be mistaken for a loadable model or appear in the picker; the finished model loads straight into the conversation. This is the one feature that touches the network, only on an explicit tap - inference, chat storage and metrics remain fully local, and the app still runs with no connection once a model is present. Importing from storage is unchanged. No inference-path, thread-derivation or pinning changes, so every published CMF and OPPO result carries over (`apk/ENTITY-v18-model-catalog-20260722-release.apk`).
- **ENTITY Bench v1.4.0** (2026-07-22): the same model catalog in the standalone benchmark app, so a phone with no `.gguf` can get a fitting one without leaving the app. Measurement, arms, sweep and CSV schema unchanged, so v1.3.0 and v1.4.0 exports stay comparable (`apk/ENTITY-Bench-v1.4.0-release.apk`).
- **ENTITY Bench v1.3.0** (2026-07-22): **optimization indicator** on the device card - a chip for every lever ENTITY ships, filled only when that lever is live on the phone in hand and a dim dashed outline otherwise, gated on the ISA flags in `/proc/cpuinfo`. It makes the difference between "ENTITY supports this" and "your phone runs this" visible before a benchmark starts. The i8mm chip deliberately claims only that the loaded ggml variant runs `MATMUL_INT8` for Q4_0/Q8_0, not a bespoke Q4_K kernel (`apk/ENTITY-Bench-v1.3.0-release.apk`).
