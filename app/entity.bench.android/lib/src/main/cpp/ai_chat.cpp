...
static bool                     g_adpf_enabled    = false;  // bench: only the adpf arm turns this on
...
// the trailing ones the efficiency cluster. The efficiency arm consumes this
// back-to-front, so a capacity-ordered list reverses to a genuine slowest-first order.
...
Java_com_arm_aichat_internal_InferenceEngineImpl_prepare(JNIEnv * /*env*/, jobject /*unused*/) {
    ...
    // cluster is a defensible default, not a measurement; the thread sweep is what should
    // confirm the value per device.
...
Java_com_arm_aichat_internal_InferenceEngineImpl_configure(
        ...
        jboolean pinCores, jboolean pinEfficiency, jboolean adpf) {
    ...
    LOGi("%s: ctx=%d threads=%d temp=%.2f topK=%d topP=%.2f pinCores=%d pinEfficiency=%d adpf=%d",
         __func__, g_n_ctx, g_n_threads, g_temp, g_top_k, g_top_p,
         (int) g_pin_cores, (int) g_pin_slow, (int) g_adpf_enabled);
}
...
Java_com_arm_aichat_internal_InferenceEngineImpl_benchModel(JNIEnv *env, jobject /*unused*/, jint pp, jint tg,
    ...
    for (nri = 0; nri < nr; nri++) {
        ...
        for (i = 0; i < tg; i++) {
            ...
            // One decode step is the unit of work the hint session is told about. Only
            // the adpf arm sets g_adpf_enabled, so on every other arm these are no-ops.
