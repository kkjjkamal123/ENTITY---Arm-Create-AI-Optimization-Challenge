...
class BenchmarkActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        when (intent.getIntExtra(EXTRA_RUNS, 0)) {
            1 -> runsGroup.check(R.id.bench_runs_1)
            3 -> runsGroup.check(R.id.bench_runs_3)
            5 -> runsGroup.check(R.id.bench_runs_5)
        }
        if (intent.getBooleanExtra(EXTRA_AUTOSTART, false) && savedInstanceState == null) {
            runBtn.post { runBenchmark() }
        }
    }
    ...
    private fun showResults(r: Result) {
        ...
        saveHistory(r, naiveTg, threadsTg, optTg)
    ...
    // One JSONL line per completed run so the home screen can show last result
    // and history without re-parsing full CSVs.
    private fun saveHistory(r: Result, naiveTg: Double, threadsTg: Double, optTg: Double) {
        runCatching {
            val line = org.json.JSONObject().apply {
                put("ts", System.currentTimeMillis())
                put("model", modelTv.text.toString())
                put("runs", r.naive.runs.size)
                put("charging", r.charging)
                put("naive", naiveTg)
                put("threads", threadsTg)
                put("auto", optTg)
            }
            java.io.File(filesDir, HISTORY_FILE).appendText(line.toString() + "\n")
        }
    }
    ...
    companion object {
        ...
        const val HISTORY_FILE = "bench_history.jsonl"
