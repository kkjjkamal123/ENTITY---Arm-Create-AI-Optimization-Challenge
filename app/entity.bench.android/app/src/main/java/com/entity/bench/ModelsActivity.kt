 ...
 *
 * Selection lives in [BenchHomeActivity], so this screen never runs anything itself: picking
 * one returns its path as an activity result and the home screen arms it for the next run,
 * so a download finishing here can hand straight over to a benchmark.
 */
